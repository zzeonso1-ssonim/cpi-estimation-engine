...
from engine.db import connect, load_buckets, load_index, published_core_sum, insert_event, load_items, load_app_state, save_app_state
...
# ── 저장된 입력 상태 초기화 (세션 첫 로드 시 1회) ─────────────────────
if "_state_loaded" not in st.session_state:
    _saved = load_app_state(con)
    def _sv(key, default):
        return _saved.get(key, default)
    # 공통 입력
    if "target_label" not in st.session_state:
        st.session_state["target_label"] = _sv("target_label", "2026년 6월")
    if "forecast_mode" not in st.session_state:
        st.session_state["forecast_mode"] = _sv("forecast_mode", "bottom-up (버킷 MoM → 재정규화)")
    # 직전월·전년동월 — 저장값이 months에 없으면 기본값 사용
    _saved_prev = _sv("prev_ym", months[-1] if months else "")
    _saved_base = _sv("base_ym", months[0] if months else "")
    if "prev_ym" not in st.session_state:
        st.session_state["prev_ym"] = _saved_prev if _saved_prev in months else (months[-1] if months else "")
    if "base_ym" not in st.session_state:
        st.session_state["base_ym"] = _saved_base if _saved_base in months else (months[0] if months else "")
    # 이벤트 관련
    if "ev_target" not in st.session_state:
        st.session_state["ev_target"] = _sv("ev_target", "2026-06")
    if "use_events" not in st.session_state:
        st.session_state["use_events"] = _sv("use_events", "True") == "True"
    if "use_service" not in st.session_state:
        st.session_state["use_service"] = _sv("use_service", "False") == "True"
    if "use_tier2" not in st.session_state and "use_tier2" in _saved:
        st.session_state["use_tier2"] = _sv("use_tier2", "True") == "True"
    # 버킷 MoM 기본값 (seokyu 제외 — 동적 키라 아래에서 별도 처리)
    for _b in load_buckets(con):
        if _b.code == "seokyu":
            continue
        _sk = f"bm_{_b.code}"
        if _sk not in st.session_state and f"bmom_{_b.code}" in _saved:
            st.session_state[_sk] = float(_saved[f"bmom_{_b.code}"])
    # 석유류 저장 기본값 — 동적 키도 함께 사전 설정
    if "seokyu_default_saved" in _saved:
        _seo_def = float(_saved["seokyu_default_saved"])
        st.session_state["_seokyu_default_saved"] = _seo_def
        _seo_widget_key = f"bm_seokyu_{_seo_def}"
        if _seo_widget_key not in st.session_state:
            st.session_state[_seo_widget_key] = _seo_def
    st.session_state["_state_loaded"] = True

target_label = st.sidebar.text_input("전망 대상월 라벨", key="target_label")
prev_ym = st.sidebar.selectbox("직전월 지수", months, key="prev_ym")
base_ym = st.sidebar.selectbox("전년동월 지수(YoY 분모)", months, key="base_ym")
...
# ── 💾 입력 저장 버튼 (모든 위젯 렌더 후 session_state 확정된 값 저장) ─────
import datetime as _dt
st.sidebar.divider()
if st.sidebar.button("💾 현재 입력 저장"):
    _state = {}
    for _k in ["target_label", "forecast_mode", "prev_ym", "base_ym",
               "ev_target", "use_events", "use_service", "use_tier2"]:
        if _k in st.session_state:
            _state[_k] = str(st.session_state[_k])
    # 버킷 MoM (seokyu 제외)
    for _b in load_buckets(con):
        if _b.code == "seokyu":
            continue
        ...
        if _sk in st.session_state:
            _state[f"bmom_{_b.code}"] = str(st.session_state[_sk])
    # 석유류 — 직전 렌더에서 기록한 동적 키로 추출
    _seo_key = f"bm_seokyu_{st.session_state.get('_cur_seokyu_default', 0.10)}"
    if _seo_key in st.session_state:
        _seo_val = st.session_state[_seo_key]
        _state["bmom_seokyu"] = str(_seo_val)
        _state["seokyu_default_saved"] = str(_seo_val)
    _state["saved_at"] = _dt.datetime.now().strftime("%Y-%m-%d %H:%M")
    save_app_state(con, _state)
    st.sidebar.success(f"저장 완료 ({_state['saved_at']})")

_saved_at = load_app_state(con).get("saved_at")
if _saved_at:
    st.sidebar.caption(f"마지막 저장: {_saved_at}")

# ── 메인 탭 ──────────────────────────────────────────────────
tab1, tab2 = st.tabs(["📊 단기 전망 (버킷 분해)", "📈 MoM 경로 시나리오"])
...
with tab1:
    if mode.startswith("direct"):
        ...
        seokyu_default = st.session_state.get("_seokyu_default_saved", 0.10)
        ...
        st.session_state["_cur_seokyu_default"] = seokyu_default

        use_events = st.checkbox("이벤트 계수 반영 (§7)", value=True, key="use_events")
        ev_target = st.text_input("이벤트 반영월 (YYYY-MM)", key="ev_target",
                                  help="이 월에 해당하는 event_coef 행 + 전월 이벤트의 되돌림이 적용됨")

        # ── 🎯 결정요인 라이브 트래킹 (⭐ 우선순위, §4.5) ──────────────────────
        bw_map = {b.code: b.weight for b in buckets}
        items_map = load_items(con)
        ...
        # 개인서비스 이중계상 방지 (§5.4)
        service_result = None
        use_service = st.checkbox("개인서비스 §5.4 정밀 계산 (이중계상 방지)", key="use_service")
        if use_service:
        ...
        # 재정규화 Tier 선택 (§5.3 / §1.3)
        use_tier2 = st.checkbox(
            f"Tier 2 정밀 재정규화 (458품목 공식 근원정의) — 품목 {len(items_map)}개",
            value=st.session_state.get("use_tier2", bool(items_map)), key="use_tier2",
...
# ════════════════════════════════════════════════════════════
# 탭 2: MoM 경로 시나리오 (과거 1년 + 향후 1년)
# ════════════════════════════════════════════════════════════
with tab2:
    import plotly.graph_objects as go
    from engine.path_engine import (
        build_path, default_scenarios, HIST_YOY, add_months, to_date_str
    )

    st.subheader("📈 MoM 경로 시나리오 — 과거 1년 + 향후 1년")
    st.caption(
        "통계청 공표 YoY(과거 1년) + 고정 MoM 가정 경로(향후 1년)를 한 차트에 표시합니다. "
        "헤드라인 · 근원② 동시 조망. 엑셀 수기모델의 Python 재현."
    )

    # ── 마지막 확정월 & 시작 지수 (DB에서 자동) ─────────────────
    last_ym = idx_df["ym"].max()
    last_row = idx_df[idx_df["ym"] == last_ym].iloc[0]
    start_h  = float(last_row["headline"])
    start_c2 = float(last_row["core2"])

    # 전년동월 분모 시리즈 (DB: 2025-05 ~ 2026-05)
    base_h_map  = {row["ym"]: float(row["headline"]) for _, row in idx_df.iterrows()}
    base_c2_map = {row["ym"]: float(row["core2"])    for _, row in idx_df.iterrows()}

    st.info(
        f"기준점: **{last_ym}** | 헤드라인 {start_h} / 근원② {start_c2}  "
        f"(전망 기간: {add_months(last_ym, 1)} ~ {add_months(last_ym, 12)})"
    )

    # ── 사용자 커스텀 입력 ────────────────────────────────────────
    with st.expander("⚙️ 시나리오 MoM 가정 조정", expanded=False):
        st.caption("이달 단독 시나리오(「이달↓→이후 0.2%」)의 이달 MoM 값을 바꿀 수 있습니다.")
        this_m_mom = st.number_input(
            "이달 MoM (이달↓ 시나리오)", value=0.1, min_value=-2.0, max_value=2.0,
            step=0.05, format="%.2f", key="path_this_mom"
        )
        st.caption("헤드라인과 근원② 모두 같은 MoM 시나리오를 적용합니다.")

    this_m_mom = st.session_state.get("path_this_mom", 0.1)

    # ── 시나리오 경로 계산 ─────────────────────────────────────
    scenario_defs = default_scenarios(this_m_mom)
    paths = []
    for s in scenario_defs:
        p = build_path(
            name=s["name"], color=s["color"], dash=s["dash"],
            moms=s["moms"],
            start_ym=last_ym, start_h=start_h, start_c2=start_c2,
            base_h=base_h_map, base_c2=base_c2_map,
        )
        paths.append(p)

    # ── 과거 1년 실측 YoY ─────────────────────────────────────
    hist_months = sorted(HIST_YOY["headline"].keys())  # 2025-06 ~ 2026-05
    hist_h  = [HIST_YOY["headline"][m] for m in hist_months]
    hist_c2 = [HIST_YOY["core2"][m]    for m in hist_months]

    # 연결점: 마지막 확정 YoY → 시나리오 시작점
    bridge_h  = HIST_YOY["headline"].get(last_ym)
    bridge_c2 = HIST_YOY["core2"].get(last_ym)

    def _make_chart(title: str, hist_vals: list, bridge_val: float,
                    path_attr: str, yrange=None) -> go.Figure:
        fig = go.Figure()

        # 실측 선
        fig.add_trace(go.Scatter(
            x=[to_date_str(m) for m in hist_months],
            y=hist_vals,
            name="실측 (통계청)",
        ...
        # 시나리오 선 (마지막 실측점에서 이어지도록 bridge 포함)
        for p in paths:
            x_vals = [to_date_str(last_ym)] + [to_date_str(m) for m in p.months]
            y_vals = [bridge_val] + getattr(p, path_attr)
            fig.add_trace(go.Scatter(
                x=x_vals, y=y_vals,
                name=p.name,
                line=dict(color=p.color, width=2, dash=p.dash),
                mode="lines",
            ))

        # 전망 시작 기준선
        fig.add_vline(
            x=to_date_str(add_months(last_ym, 1)),
            line_dash="dot", line_color="gray", line_width=1,
            annotation_text="전망 시작", annotation_position="top right",
        )

        fig.update_layout(
            title=title, yaxis_title="YoY (%)",
    ...
    # ── 차트 렌더링 ────────────────────────────────────────────
    col_h, col_c2 = st.columns(2)
    with col_h:
        fig_h = _make_chart(
            "헤드라인 CPI YoY (%)", hist_h, bridge_h, "headline_yoys"
        )
        st.plotly_chart(fig_h, use_container_width=True)
    with col_c2:
        fig_c2 = _make_chart(
            "근원② CPI YoY (%)", hist_c2, bridge_c2, "core2_yoys"
        )
        st.plotly_chart(fig_c2, use_container_width=True)

    # ── 상세 수치 테이블 ──────────────────────────────────────
    st.divider()
    st.markdown("**월별 YoY 상세 (향후 1년)**")

    # 헤드라인 테이블
    tbl_rows = []
    for i, p in enumerate(paths):
        for j, ym in enumerate(p.months):
            tbl_rows.append({
                "월": ym,
                p.name: round(p.headline_yoys[j], 2),
            })

    # pivot: 월 × 시나리오
    tbl_df = pd.DataFrame({"월": paths[0].months})
    for p in paths:
        tbl_df[p.name] = [round(v, 2) for v in p.headline_yoys]

    st.markdown("*헤드라인 CPI YoY (%)*")
    st.dataframe(tbl_df, hide_index=True, use_container_width=True)

    tbl_c2 = pd.DataFrame({"월": paths[0].months})
    for p in paths:
        tbl_c2[p.name] = [round(v, 2) for v in p.core2_yoys]

    st.markdown("*근원② CPI YoY (%)*")
    st.dataframe(tbl_c2, hide_index=True, use_container_width=True)

    # ── 연간 평균 YoY (2026년) ────────────────────────────────
    st.divider()
    st.markdown("**2026년 연간 평균 YoY 추정**")
    st.caption(
        "1~5월 확정(통계청) + 6~12월 시나리오 경로 가중평균. "
        "단, 헤드라인 기준."
    )

    confirmed_2026 = {m: v for m, v in HIST_YOY["headline"].items() if m.startswith("2026-")}
    avg_rows = []
    for p in paths:
        forecast_2026 = {ym: yoy for ym, yoy in zip(p.months, p.headline_yoys)
                         if ym.startswith("2026-")}
        all_2026 = {**confirmed_2026, **forecast_2026}
        avg = sum(all_2026.values()) / len(all_2026) if all_2026 else None
        avg_rows.append({"시나리오": p.name, "2026년 평균 YoY (%)": round(avg, 2) if avg else "—"})

    st.dataframe(pd.DataFrame(avg_rows), hide_index=True, use_container_width=True)

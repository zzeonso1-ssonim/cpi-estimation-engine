...
if mode.startswith("direct"):
    ...
    recs = st.session_state.get("opinet")
    if recs:
        cur_ym = recs[-1]["date"][:7]
        mavg = gasoline_monthly_avg(recs, ym=cur_ym)
        if mavg:
            oc2.success(
                f"오피넷 **{mavg['ym']} 당월 평균** 보통휘발유 **{mavg['avg']}원/L** "
                f"({mavg['n_days']}일 평균 · {mavg['first_date']}~{mavg['last_date']})"
            )
            base_oil = oc2.number_input(
                "전월 오피넷 보통휘발유 평균(원/L)", value=2009.08, step=1.0,
                help="전월 일별 평균가 입력 → 석유류 MoM = 당월평균 / 전월평균 − 1")
            if base_oil:
                seokyu_default = round((mavg["avg"] / base_oil - 1) * 100, 2)
                oc2.caption(
                    f"→ 석유류 MoM 자동 제안 **{seokyu_default:+.2f}%** "
                    f"(당월평균 {mavg['avg']} / 전월평균 {base_oil})"
                )
